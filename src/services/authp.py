...
class AuthItmoIdService:
    ...
    async def login(self, username: str, password: str) -> TokenResponse:
        ...
        return TokenResponse(
            refresh_token=self._refresh_token, access_token=self._access_token
        )
    ...
    def _generate_pkce():
        ...
        challenge = (
            base64.urlsafe_b64encode(challenge_hash).decode("utf-8").replace("=", "")
        )
    ...
    async def _fetch_login(self):
        ...
        initial_resp = await self._session.get(
            "/auth", params=params, follow_redirects=True
        )
        # self.logger.log_response(initial_resp)
        if not initial_resp.is_success:
            raise self.MainException(
                f"Login step 1 failed: with status code {initial_resp.status_code}"
            )
        ...
        if not match:
            raise self.MainException(
                "Login step 1 failed: could not find loginActionUrl in page source"
            )
        ...
        auth_resp = await self._session.post(
            login_action_url, data=auth_data, follow_redirects=False
        )
        ...
        if not location:
            raise self.MainException(
                "Login step 2 failed: 'Location' header is missing (check credentials)"
            )
        ...
        if not code:
            raise self.MainException(
                "Login step 2 failed: auth code not found in redirect URL"
            )
        ...
        if not token_resp.is_success:
            raise self.MainException(
                f"Login step 3 failed: with status code {token_resp.status_code}"
            )
...
async def get_authp_service() -> AsyncGenerator[AuthItmoIdService, None]:
    yield AuthItmoIdService()
